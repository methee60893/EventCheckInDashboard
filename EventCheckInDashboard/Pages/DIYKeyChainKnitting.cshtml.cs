...
{
    public class DIYKeyChainKnittingModel : PageModel
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<DIYKeyChainKnittingModel> _logger;
        ...
        public DIYKeyChainKnittingModel(IConfiguration configuration, ILogger<DIYKeyChainKnittingModel> logger)
        {
        ...
        public async Task OnGetAsync()
        {
            ...
            {
                _logger.LogError(ex, "An error occurred while fetching DIYKeyChainKnitting dashboard data.");
                ErrorMessage = "เกิดข้อผิดพลาดในการดึงข้อมูลสำหรับหน้า DIYKeyChainKnitting";
            }
        }

        private async Task LoadSummaryAutualRightDataAsync(string connectionString, int stationId)
        {
            string sql = "SELECT CAST(SUM(CASE WHEN RewardTypeID = 14 THEN FLOOR(Carat/800.0) ELSE 0 END) AS INT) AS TotalRights FROM [dbo].[RewardHistory] WHERE [StationId]=@StationId ;";
        ...
        private async Task LoadChartDataAsync(string connectionString, int stationId)
        {
            ...
            string barSql = @"
                SELECT CAST(UsedAt AS DATE) as ActivityDate,
                    CAST(SUM(CASE WHEN RewardTypeID = 14 THEN FLOOR(Carat/800.0) ELSE 0 END) AS INT) AS Karat800
            ...
            await using (var connection = new SqlConnection(connectionString))
            {
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    dailyCounts[date] = ((int)reader["Karat800"]);
                }
            }
            ...
            var barData = new
            {
                ...
                datasets = new object[]
                {
                    new { label = "แลก800กะรัต", data = labels.Select(l => dailyCounts.ContainsKey(l) ? dailyCounts[l] : 0).ToList(), backgroundColor = "rgba(234, 179, 8, 0.85)" }
                }
            };
            ...
            string sql = @"-- ประกาศตัวแปร
                        ...
                        WITH SourceData AS (
                            SELECT 
                                ...
                                CASE 
                                    WHEN RewardTypeID = 1 THEN N''แลก800กะรัต''
                                    ELSE N''อื่นๆ'' 
                                END AS Segment,
                                CASE 
                                    WHEN RewardTypeID = 1 THEN CAST(FLOOR(ISNULL(Carat, 0) / 800.0) AS INT)
                                    ELSE 1 

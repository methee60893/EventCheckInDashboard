...
namespace EventCheckInDashboard.Pages
{
    public class DIYKeyChainKnittingModel : PageModel
    {
        ...
        {
            string sql = @"SELECT CAST(SUM(CASE WHEN RewardTypeID = 14 THEN FLOOR(Carat/800.0) ELSE 0 END) AS INT) AS TotalRights 
            ...
            string sql = @"-- ประกาศตัวแปร
                        ...
                        WITH SourceData AS (
                            SELECT 
                                ...
                                CASE 
                                    WHEN RewardTypeID = 14 THEN N''แลก800กะรัต''
                                    ELSE N''อื่นๆ'' 
                                END AS Segment,
                                CASE 
                                    WHEN RewardTypeID = 14 THEN CAST(FLOOR(ISNULL(Carat, 0) / 800.0) AS INT)
                                    ELSE 1 

    ...
    public class MatchaBarLoverModel : PageModel
    {
        ...
        public async Task OnGetAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                ErrorMessage = "เกิดข้อผิดพลาดในการดึงข้อมูลสำหรับหน้า MatchaBarLover";
            }
        }

        private async Task LoadSummaryAutualRightDataAsync(string connectionString, int stationId,string startDate,string endDate)
        {
            string sql = "SELECT CAST(SUM(CASE WHEN RewardTypeID = 15 THEN FLOOR(Carat/400.0) ELSE 0 END) AS INT) AS TotalRights FROM [dbo].[RewardHistory] WHERE [StationId]=@StationId ;";
        ...
        private async Task LoadChartDataAsync(string connectionString, int stationId, string startDate, string endDate)
        {
            ...
            string barSql = @"
                SELECT CAST(UsedAt AS DATE) as ActivityDate,
                    CAST(SUM(CASE WHEN RewardTypeID = 15 THEN FLOOR(Carat/400.0) ELSE 0 END) AS INT) AS Karat400
            ...
            await using (var connection = new SqlConnection(connectionString))
            {
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    dailyCounts[date] = ((int)reader["Karat400"]);
                }
            }
            ...
            var barData = new
            {
                ...
                datasets = new object[]
                {
                    new { label = "แลก400กะรัต", data = labels.Select(l => dailyCounts.ContainsKey(l) ? dailyCounts[l] : 0).ToList(), backgroundColor = "rgba(234, 179, 8, 0.85)" }
                }
            };
            ...
            string sql = @"-- ประกาศตัวแปร
                        ...
                        WITH SourceData AS (
                            SELECT 
                                ...
                                CASE 
                                    WHEN RewardTypeID = 15 THEN N''แลก400กะรัต''
                                    ELSE N''อื่นๆ'' 
                                END AS Segment,
                                ...
                                CASE 
                                    WHEN RewardTypeID = 15 THEN CAST(FLOOR(ISNULL(Carat, 0) / 400.0) AS INT)
                                    ELSE 1 

...
namespace EventCheckInDashboard.Pages
{
    public class SpinwheelModel : PageModel
    {
        ...
        private async Task LoadSummaryDataAsync(string connectionString, int stationId)
        {
            ...
            string sql = "SELECT COUNT(MemberRewardID) AS TotalRights, COUNT(DISTINCT MemberID) AS TotalMembers FROM [dbo].[MemberRewards] WHERE StationId = @stationId;";
        ...
        private async Task LoadSegmentPivotTableAsync(string connectionString, int stationId)
        {
            string sql = @"
                ...
                SELECT @cols = STUFF((SELECT DISTINCT ',' + QUOTENAME(CAST(CreatedAt AS DATE)) FROM [dbo].[MemberRewards] WHERE StationId = @stationId FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,1,'');
                IF @cols IS NULL RETURN;
                SELECT @totalCol = STUFF((SELECT DISTINCT ' + ISNULL(' + QUOTENAME(CAST(CreatedAt AS DATE)) + ', 0)' FROM [dbo].[MemberRewards] WHERE StationId = @stationId FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,2,'');
                SELECT @colsForSelect = STUFF((SELECT DISTINCT ', ISNULL(' + QUOTENAME(CAST(CreatedAt AS DATE)) + ', 0) AS ' + QUOTENAME(FORMAT(CreatedAt, 'dd MMM')) FROM MemberReward WHERE StationId = @stationId FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,1,'');
        ...
        private async Task LoadTierPivotTableAsync(string connectionString, int stationId)
        {
            string sql = @"
                ...
                WITH PivotedData AS (
                    SELECT Tier, ' + @cols + '
                    FROM (SELECT Tier, CAST(CreatedAt AS DATE) AS ActivityDate FROM [dbo].[MemberRewards] WHERE RewardTypeID IN (1,2,3)) AS SourceTable
                    PIVOT (COUNT(ActivityDate) FOR ActivityDate IN (' + @cols + ')) AS pvt
                )

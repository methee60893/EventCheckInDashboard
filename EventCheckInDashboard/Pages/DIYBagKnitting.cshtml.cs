    ...
    public class DIYBagKnittingModel : PageModel
    {
        ...
        private async Task LoadChartDataAsync(string connectionString, int stationId)
        {
            ...
            var barData = new
            {
                ...
                datasets = new object[]
                {
                    new { label = "ใบเสร็จ 20,000", data = labels.Select(l => dailyCounts.ContainsKey(l) ? dailyCounts[l] : 0).ToList(), backgroundColor = "rgba(234, 179, 8, 0.85)" }
                }
            };
            ...
            string sql = @"-- ประกาศตัวแปร
                        ...
                        WITH SourceData AS (
                            SELECT 
                                ...
                                CASE 
                                    WHEN RewardTypeID = 16 THEN N''ใบเสร็จ 20,000''
                                    ELSE N''อื่นๆ'' 
                                END AS Segment,

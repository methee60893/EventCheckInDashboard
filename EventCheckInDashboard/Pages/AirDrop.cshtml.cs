...
{
    public class AirDropModel : PageModel
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<AirDropModel> _logger;
        ...
        public AirDropModel(IConfiguration configuration, ILogger<AirDropModel> logger)
        {
        ...
        public async Task OnGetAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                ErrorMessage = "เกิดข้อผิดพลาดในการดึงข้อมูลสำหรับหน้า AirDrop";
            }
        }
        private async Task LoadSummaryAutualRightDataAsync(string connectionString, int stationId, string startDate, string endDate)
        {
            string sql = @"SELECT SUM(CASE WHEN RewardTypeID IN (9,10,11) THEN 1 ELSE 0 END) AS TotalRights 
        ...
        private async Task LoadChartDataAsync(string connectionString, int stationId, string startDate, string endDate)
        {
            ...
            string barSql = @"
                SELECT CAST(UsedAt AS DATE) as ActivityDate,
                    ...
                    SUM(CASE WHEN RewardTypeID = 9 THEN 1 ELSE 0 END) AS SoftServe
                FROM [dbo].[RewardHistory] 
                WHERE StationId = @stationId
                AND  CAST([UsedAt] AS DATE) BETWEEN @startDate AND @endDate
